...
/* An HTTP GET handler to serve the captured image */
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(server, &pic); // Register the /pic handler
...
void app_main(void)
{
    ...
    while (server) {
        vTaskDelay(1000); // Sleep for a while
    }
}
